...
# and check the code coverage of the package gym_breakout_pygame.some_package:
#
#     make test-sub tdir=some_package dir=gym_breakout_pygame.some_package
#
.PHONY: test-sub
test-sub:
	pytest -rfE tests/test_$(tdir) --cov=gym_breakout_pygame.$(dir) --cov-report=html --cov-report=xml --cov-report=term-missing --cov-report=term  --cov-config=.coveragerc
	find . -name ".coverage*" -not -name ".coveragerc" -exec rm -fr "{}" \;
